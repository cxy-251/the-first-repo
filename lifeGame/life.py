...
class Life:
    def __init__(self):
        ...
        # screen.blit =
        # set_mode需要传入参数是关键词参数,指定size,缺省关键词默认
        # 悬停或F12查看
        # >>> SCR_RECT
        # <rect(0, 0, 900, 600)>
        ...
        # NON_MARKED, WIDTH, HEIGHT = 3, 90, 60
        ...
        # COL_0, WIDTH, WIDTH, HEIGHT = 14, 90, 60
        # cursor = [52, 30]

        # make a clock instance实例化
        ...
        for x in range(WIDTH):
            ...
            # pygame.draw.rect(surface, color, rect) -> Rect其他参数缺省
            # Rect(left, top, width, height) -> Rect
    ...
    def loadColor(self, num_pattern):
        ...
        NEW_ALIVE_COLOR = self.c1
        DEAD_COLOR = self.c3
        # 单个文件内,全局变量,global类变量,global实例变量,global局部变量,等等,修改了就是全局修改。     #多个文件内就不是这样
    ...
    def next(self):
        ...
        for y in range(ROW_0, HEIGHT - 1):
            for x in range(COL_0, WIDTH - 1):
                ...
                # univ里只存放两种状态DEAD=0&&ALIVE=1
                if self.univ[y][x] == ALIVE or self.hist[y][x] == DIED_OUT:
                    self.hist[y][x] = MARKED
                    # hist里存放MARKED=2&&NON_MARKED=3&&NEW_ALIVE&&DIED_OUT
